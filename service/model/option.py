...
class ReserveOption(db.Model):
    ...
    owner_id = db.Column(
        db.Integer, db.ForeignKey("owner.id", ondelete="CASCADE"), nullable=False
    )
...
class ClientReservationList(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True, unique=True, nullable=False)
    option_id = db.Column(
        db.Integer, db.ForeignKey("options.id", ondelete="CASCADE"), nullable=False
    )
    client_id = db.Column(
        db.Integer, db.ForeignKey("client.id", ondelete="CASCADE"), nullable=False
    )
    reserved_date = db.Column(db.DateTime(timezone=True))
    expired_date = db.Column(db.DateTime(timezone=True))
